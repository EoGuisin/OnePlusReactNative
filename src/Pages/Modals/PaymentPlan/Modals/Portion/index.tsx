...
interface Props {
  ...
  setVisibleModalPortion:(value: boolean) => void,
}

dayjs.extend(require('dayjs/plugin/customParseFormat'));

export const Portion = (props: Props, Type: Type.ViewModel) => {
  //#region useState
  ...
  async function pickImageFromCamera() {
    ...
    if (result?.assets) {
      let annex = {
        ...
        descricao: `14. Comp de pagamento (${props.getTitulosDeSaldo.titulo.toLowerCase()})`,
        extensao: "jpg",
      }
      props.setTitulosDeSaldo((prevState: Type.ViewModel) => {
  ...
  const calcularDescontoMaximo = () => {
    switch (SelectedCompany?.id) {
      case 4:
        if (props.getTitulosDeSaldo.titulo == "Entrada") return 0;
        else return 15;
      default:
        return 100;
    }
  ...
  const debounceUltimaParcela = useDebouncedCallback((titulo: Type.Title) => {
    let valorTotalTitulos = considerar2CasasDecimais(props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => {
    ...
    hookSetViewModel({ ...props.getTitulosDeSaldo, titulos: [...props.getTitulosDeSaldo.titulos] })
  }, 500);

  const calcularDesconto = (valueNumber: number) => {
    if (props.getTitulosDeSaldo.totalEsperadoBase) {
      let titulo = props.getTitulosDeSaldo.titulos.find((Item) => Item.valorTotal !== 0)
      if (!titulo) return;
      props.getTitulosDeSaldo.totalEsperado = considerar2CasasDecimais(props.getTitulosDeSaldo.totalEsperadoBase - (props.getTitulosDeSaldo.totalEsperadoBase * (valueNumber !== 0 ? (valueNumber / 100) : 0)))
      props.setValorDescontadoTituloSaldo(props.getTitulosDeSaldo.totalEsperadoBase - props.getTitulosDeSaldo.totalEsperado)
      let totalEsperadoJuros
      if (props.getTitulosDeSaldo.totalEsperadoBaseJuros) {
          totalEsperadoJuros = considerar2CasasDecimais(props.getTitulosDeSaldo.totalEsperadoBaseJuros - (props.getTitulosDeSaldo.totalEsperadoBaseJuros * (valueNumber !== 0 ? (valueNumber / 100) : 0)))
      }
      props.getTitulosDeSaldo.totalEsperadoJuros = totalEsperadoJuros ?? props.getTitulosDeSaldo.totalEsperado
      if (props.getTitulosDeSaldo.titulos.length === 1) {
          titulo.valorTotal = props.getTitulosDeSaldo.totalEsperado!
          titulo.valorTotalJuros = totalEsperadoJuros ?? props.getTitulosDeSaldo.totalEsperado!
      } else {
          const indexTitulo = props.getTitulosDeSaldo.titulos.findIndex((Item) => Item === titulo)
          titulo.valorTotal = considerar2CasasDecimais(props.getTitulosDeSaldo.totalEsperado! - props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual, idx) => Acumulado + (idx === indexTitulo ? 0 : Atual.valorTotal), 0))
          titulo.valorTotalJuros = totalEsperadoJuros ?? considerar2CasasDecimais(props.getTitulosDeSaldo.totalEsperado! - props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual, idx) => Acumulado + (idx === indexTitulo ? 0 : Atual.valorTotalJuros), 0))
      }
  ...
  function Quantidade(value: string, Item: Type.Title) {
    let UltimoTitulo = [...props.getTitulosDeSaldo.titulos].sort((a, b) => b.valor - a.valor).sort((a, b) => dayjs(a.vencimento).add(a.quantidade - 1, 'months').toDate().getTime() - dayjs(b.vencimento).add(b.quantidade - 1, 'months').toDate().getTime())[props.getTitulosDeSaldo.titulos.length - 1];
    ...
    if (Item.valorTotal == 0 || !Item.valorTotal) {
      Item.valorTotal = Qtde === 0 ? 0 : considerar2CasasDecimais((props.getTitulosDeSaldo.totalEsperado ?? 0) - props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual, index) => {
        if (Atual === Item) return Acumulado;
        if (Atual.teveAjuste && !!Atual.valorUltimaParcela) return Acumulado + (Atual.valorUltimaParcela + (Atual.valor * (Atual.quantidade - 1)))
        return Acumulado + Atual.valorTotal
      }, 0))
    }
    Item.valor = considerar2CasasDecimais(SelectedCompany?.id == 6 ? CalculosFinanceiros.PMT(props.getTitulosDeSaldo.jurosDeTabela ?? 0, Qtde, props.getTitulosDeSaldo.pv ?? 0, 0, 0) : Item.valorTotal / Item.quantidade !== Infinity ? Item.valorTotal / Item.quantidade : 0);
    if (Item.sistemaDeAmortizacao === 1 && Item.juros > 0) {
      Item.valorJuros = considerar2CasasDecimais(- CalculosFinanceiros.PMT(Item.juros, props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => Acumulado + Atual.quantidade, 0), props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => Acumulado + Atual.valorTotal, 0), 0, 0));
    ...
    hookSetViewModel({ ...props.getTitulosDeSaldo, titulos: [...props.getTitulosDeSaldo.titulos] });
    const qtdeTotal = props.getTitulosDeSaldo.titulos.reduce((acumulado, atual) => acumulado + atual.quantidade, 0);
    if (props.getTitulosDeSaldo.maiorQuantidade && qtdeTotal > props.getTitulosDeSaldo.maiorQuantidade) {
  ...
  function Maturity(date: Date, Item: Type.Title) {
    ...
    let Titulos = [...props.getTitulosDeSaldo.titulos];
    Item.vencimento = dataSelecionada;
    debounceUltimaParcela(Item);
    hookSetViewModel({ ...props.getTitulosDeSaldo, titulos: [...Titulos] });
  }
  ...
  function DeleteTitle(index: number) {
    if(props.getTitulosDeSaldo.titulos.length == 1) {
      ...
      props.getTitulosDeSaldo.titulos.splice(index, 1);
    }
    props.setTitulosDeSaldo(props.getTitulosDeSaldo)
  }

  function NewTitle() {
    if (props.getTitulosDeSaldo.titulos && props.getTitulosDeSaldo.titulos.length > 0) {
      const Titulo = Object.assign({}, props.getTitulosDeSaldo.titulos[0]);
      const novoTitulo = {
        quantidade: 0,
        valor: 0,
        valorJuros: 0,
      ...
      hookSetViewModel({ ...props.getTitulosDeSaldo, titulos: [...props.getTitulosDeSaldo.titulos, novoTitulo] });
    }
  }

  function DigitoCartao(value: string, index: number) {
    let TitleList = [... props.getTitulosDeSaldo.titulos];
  ...
  console.log(JSON.stringify(props.getTitulosDeSaldo.titulos))
                  ...
                  <Styled.Container>
                    <View style={{top: -10}}>
                      ...
                      <Styled.ContainerHeader style={{ marginTop: Platform.OS === 'ios' ? '5%' : '10%', opacity: getShowToast ? 0 : 1}}>
                          <TouchableOpacity activeOpacity={0.9} onPress={() => props.setVisibleModalPortion(false)}>
                              <SvgCss xml={ArrowBack} />
                          </TouchableOpacity>
                          <Styled.TextHeader>{props.getTitulosDeSaldo.titulo}</Styled.TextHeader>
                        ...
                        <Modals.ViewImage
                          ...
                          image={props.getTitulosDeSaldo.anexo?.arquivo}
                        />
                        <Modals.PdfViewer
                          visible={getVisiblePDF}
                          onPressClose={() => {setVisiblePDF(false)}}
                          resource={props.getTitulosDeSaldo.anexo?.arquivo}
                        />
                        <Annex
                          viewImage={() => setVisibleImage(true)}
                          viewPDF={() => setVisiblePDF(true)}
                          PDForIMAGE={props.getTitulosDeSaldo.anexo?.extensao == 'pdf' ? 1 : 0}
                          cancelTouch={() => {
                            props.setTitulosDeSaldo((prevState: Type.ViewModel) => {
                          ...
                          uri={props.getTitulosDeSaldo.anexo?.arquivo}
                          ...
                          {props.getTitulosDeSaldo.titulos.map((item, index) => 
                            <Styled.ContainerTitle>
                              ...
                              <DatePicker
                                modal
                                mode='date'
                                maximumDate={props.getTitulosDeSaldo.maiorVencimento}
                                minimumDate={props.getTitulosDeSaldo.menorVencimento}
                                open={getOpen}
                                date={getDate}
                                onConfirm={(date) => {
                                  Maturity(date, item)
                        ...
                        {SelectedCompany?.id != 26 && props.getTitulosDeSaldo && props.getTitulosDeSaldo.titulos.length > 0 &&
                          <View style={{marginLeft: '5%'}}>
                            <Styled.Info>* Taxa de juros aplicada de {(props.getTitulosDeSaldo.titulos[0].juros * 100).toFixed(2).replace(".", ",")}%</Styled.Info>
                            <Styled.Info>* Amortização {props.getTitulosDeSaldo.titulos[0].sistemaDeAmortizacao == 0 ? "Gradiente" : (props.getTitulosDeSaldo.titulos[0].sistemaDeAmortizacao == 1 ? "Price" : (props.getTitulosDeSaldo.titulos[0].sistemaDeAmortizacao == 2 ? "SAC" : "Simples"))}</Styled.Info>
                          </View>}
                          {props.getTitulosDeSaldo.titulos && props.getTitulosDeSaldo.titulos.length > 0 && 
                          <View style={{marginLeft: '5%'}}>
                            <Styled.Info>
                            * Segundo o modelo de vendas, o valor esperado seria de {TextFormat.Moeda.FormatarTexto(considerar2CasasDecimais(props.getTitulosDeSaldo.totalEsperado ?? 0))}.
                            O total gerado foi de {TextFormat.Moeda.FormatarTexto(considerar2CasasDecimais(props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => Acumulado + Atual.valorTotal, 0)))}.
                            {(considerar2CasasDecimais(props.getTitulosDeSaldo.totalEsperado ?? 0) == considerar2CasasDecimais(props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => Acumulado + Atual.valorTotal, 0)) ? "" :
                                (considerar2CasasDecimais(props.getTitulosDeSaldo.totalEsperado ?? 0) <= considerar2CasasDecimais(props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => Acumulado + Atual.valorTotal, 0))
                                    ? ` Logo, seguindo o modelo de vendas seria necessário ainda descontar ${TextFormat.Moeda.FormatarTexto(considerar2CasasDecimais(props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => Acumulado + Atual.valorTotal, 0) - (props.getTitulosDeSaldo.totalEsperado ?? 0)))}.`
                                    : ` Logo, seguindo o modelo de vendas seria necessário ainda distribuir ${TextFormat.Moeda.FormatarTexto(considerar2CasasDecimais(props.getTitulosDeSaldo.totalEsperado ?? 0) - considerar2CasasDecimais(props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => Acumulado + Atual.valorTotal, 0)))}.`)
                            )}
                            </Styled.Info>
                          </View>}
                          {(props.getAjustarSaldoAoValorAVista == true && (considerar2CasasDecimais(props.getTitulosDeSaldo.totalEsperado ?? 0) != considerar2CasasDecimais(props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => Acumulado + Atual.valorTotal, 0)))) &&
                          <View>
                            {props.getTitulosDeSaldo.titulos && props.getTitulosDeSaldo.titulos.length > 0 &&
                            <Styled.Info>
                              {(() => {
                                  let UltimoTitulo = [...props.getTitulosDeSaldo.titulos].sort((a, b) => b.valor - a.valor).sort((a, b) => dayjs(a.vencimento).add(a.quantidade - 1, 'months').toDate().getTime() - dayjs(b.vencimento).add(b.quantidade - 1, 'months').toDate().getTime())[props.getTitulosDeSaldo.titulos.length - 1];
                                  let Parcela = considerar2CasasDecimais(props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => Acumulado + Atual.quantidade, 0));
                                  let Vencimento = dayjs(UltimoTitulo.vencimento).add(UltimoTitulo.quantidade - 1, 'months').toDate();
                                  let Valor = Math.abs(considerar2CasasDecimais(UltimoTitulo.valor + considerar2CasasDecimais(props.getTitulosDeSaldo.totalEsperado ?? 0) - props.getTitulosDeSaldo.titulos.reduce((Acumulado, Atual) => Acumulado + Atual.valorTotal, 0)));
                                  return `Como a opção "Ajustar a última parcela?" foi marcada, o valor da parcela de nº ${TextFormat.Numero.FormatarTextoParaInteiro(Parcela)} cujo o vencimento se dará em ${dayjs(Vencimento).format("DD/MM/YYYY")} será de ${TextFormat.Moeda.FormatarTexto(Valor ?? 0)} afim de atender ao valor esperado de ${TextFormat.Moeda.FormatarTexto(props.getTitulosDeSaldo.totalEsperado ?? 0)}`;
                              })()}
                            </Styled.Info>}
                          </View>}
                        </Styled.ContainerInfo>
