...
export const ChecklistRegistration = (props: Props) => {
  ...
  async function pickImageFromCamera() {
    ...
    if(result?.assets) {
      props.setDocumentChecklist(result.assets[0].uri ? result.assets[0].uri : "")
      props.setPDForImageChecklist(0)
    }
  }
  ...
  async function DocumentPDF() {
    try {
      ...
      RNFetchBlob.fs.readFile(decodedPath, 'base64').then((data: string) => {
        console.log(data);
        props.setDocumentChecklist(data)
        props.setPDForImageChecklist(1)
      }).catch((err) => {
      ...
      <LinearGradient colors={['#26A77C', '#105B74']}>
        <Styled.Container>
          <Styled.ContainerHeader style={{marginTop: Platform.OS === 'ios' ? '15%' : '5%'}}>
            <TouchableOpacity activeOpacity={0.9} onPress={() => props.setVisibleChecklistRegistration(false)}>
              <SvgCss xml={Home} />
            </TouchableOpacity>
            <Styled.TextHeader>Checklist de Cadastro</Styled.TextHeader>
          ...
          <Styled.SubContainer>
            <Modals.ViewImage
              ...
              image={props.getDocumentChecklist}
            />
            <Modals.PdfViewer
              visible={getVisiblePDF}
              onPressClose={() => {setVisiblePDF(false)}}
              resource={props.getDocumentChecklist}
            />
            <Annex
              viewImage={() => setVisibleImage(true)}
              viewPDF={() => setVisiblePDF(true)}
              PDForIMAGE={props.getPDForImageChecklist}
              cancelTouch={() => props.setDocumentChecklist("")}
              library={() => pickImageFromGalery()}
              pdf={() => DocumentPDF()}
              camera={() => requestCameraPermission()}
              uri={props.getDocumentChecklist}
          ...
          <Styled.ContainerSubmit>
            <Styled.Submit onPress={async () => props.setVisibleProponent(true)} activeOpacity={0.8}>
            ...
            <Modals.Proponent
              ...
              visible={props.getVisibleProponent}
